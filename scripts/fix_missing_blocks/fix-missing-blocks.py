...
# Config
# should be set to whatever the name of your chainname.config.json is
chainname = "libre"
home_dir = Path.home()
filepath = f"{home_dir}/hyperion-history-api/chains/{chainname}.config.json"
indexer_stop = ["pm2", "trigger", f"{chainname}-indexer", "stop"]
indexer_start = ["./run.sh", f"{chainname}-indexer"]
indexer_log_file = f"{home_dir}/.pm2/logs/{chainname}-indexer-out.log"
tail_indexer_logs = ["tail", "-f", indexer_log_file]
es_search_index = f"{chainname}-block-*"
...
def get_buckets(interval, query):
    ...
    result = es.search(index=es_search_index, body=query)
...
def CreateGtLT(missing):
    ...
    for num, bucket in enumerate(missing):
        gte = bucket['key']
        lte = gte+lte
        result = es.search(index=es_search_index,
                           body=query_body2(gte, lte, 1000))
        buckets = result['aggregations']['block_histogram']['buckets']
